...
1. **下载便携存档**
   - 下载最新的 `.zip`（Windows）或 `.tar.gz`（macOS/Linux）存档
...
- **主内容区域**：以网格或列表视图显示图像
- **图像详细信息**：选择图像时显示元数据和属性
- **工具栏**：访问导入、排序、筛选和视图选项
- **状态栏**：显示总图像数、当前筛选器和应用程序状态
...
##### 多张图像
1. 使用 Ctrl/Cmd + 点击选择多个图像
2. 右键单击并选择 **添加标签**
3. 输入以逗号分隔的标签名称
4. 点击 **添加** 将标签应用于所有选定的图像
...
应用程序使用 SQLite 进行数据存储。数据库文件自动创建于：
- Windows：`%APPDATA%/AIGenManager/aigenmanager.db`
- macOS：`~/.local/share/AIGenManager/aigenmanager.db`
- Linux：`~/.local/share/AIGenManager/aigenmanager.db`
...
- **问题**：[https://github.com/berry-aigc-toolbox/berry-aigc-toolbox/issues](https://github.com/berry-aigc-toolbox/berry-aigc-toolbox/issues)
